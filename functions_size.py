...
def assign_values(mG, faultyNodes, faultyNodesIdx):
    ...
    for i in range(nCommunities):
        ...
        bv.append(list(tmp))
        S1 = {faultyNodesIdx[i][f]: 1 for f in range(len(bv[-1]))}  # gSize[i]+1 for f in range(len(bv[-1]))}
        S2 = {indices[g]: 1 for g in range(len(gv[-1]))}
        S1.update(S2)
        Sgood.append(S2)
    ...
    nx.set_node_attributes(mG, attributes, 'sizeTokens')
    return S, mG, Sgood
...
def plot_growth(t, r, mStr, n):
    t = range(int(t[0]))
    plt.plot(t, t, label=f'T')
    plt.plot(t, r[0], label=f'n = {gSize[0]}')
    plt.plot(t, r[1], label=f'n = {gSize[1]}')
    plt.legend(loc='best')
    plt.xlabel('T')
    plt.ylabel(f'Max Token')
    plt.title('Max token increase against time')
    plt.savefig(mStr + '.png', dpi=300)
